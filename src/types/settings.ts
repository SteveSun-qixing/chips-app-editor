/**
 * 编辑引擎设置系统类型定义
 * @module types/settings
 *
 * 设置系统采用「注册中心」架构，每个设置分类是一个完全自治的模块，
 * 包含自己的数据 schema、默认值、面板组件和变更处理器。
 *
 * 核心类型：
 * - SettingsPanelDefinition: 面板定义，是注册中心的基本单元
 * - SettingsCategory: 菜单元数据
 * - SettingsChangeEvent: 设置变更事件
 *
 * 新增设置分类的步骤：
 * 1. 定义面板组件 XxxPanel.vue
 * 2. 在 settings-registry.ts 中声明 SettingsPanelDefinition
 * 3. 完成。菜单自动出现，数据自动管理。
 */

import type { ComponentType } from 'react';

// ============================================================
// 设置分类标识与元数据
// ============================================================

/**
 * 设置分类标识
 *
 * 使用 string 类型以支持无限扩展。
 * 内置分类使用固定字符串（language、theme、layout、tools、fileMode、about）。
 */
export type SettingsCategoryId = string;

/**
 * 设置分类菜单分组
 *
 * 用于在左侧菜单栏中进行视觉分组。
 * 同一 group 的分类之间无分隔线，不同 group 之间显示分隔线。
 */
export type SettingsCategoryGroup = string;

/**
 * 设置分类元数据
 *
 * 描述一个设置分类在菜单中的显示信息。
 */
export interface SettingsCategory {
  /** 分类唯一标识 */
  id: SettingsCategoryId;
  /** 分类名称的多语言 key（通过 t() 函数获取显示文本） */
  labelKey: string;
  /** 分类图标（emoji 或图标标识） */
  icon?: string;
  /** 排序权重，越小越靠前 */
  order: number;
  /** 菜单分组标识（可选，用于菜单分隔线） */
  group?: SettingsCategoryGroup;
}

// ============================================================
// 面板定义 —— 注册中心的核心类型
// ============================================================

/**
 * 设置面板定义
 *
 * 每个设置分类的完整描述。面板定义是注册中心的基本单元，
 * 包含该分类的所有信息，使其成为一个完全自治的模块。
 *
 * 泛型参数 T 表示该分类的设置数据类型，
 * 保证 defaultData 和 onChange 的类型安全。
 *
 * @template T - 该分类的设置数据类型
 *
 * @example
 * ```typescript
 * const themePanel: SettingsPanelDefinition<ThemeSettingsData> = {
 *   category: {
 *     id: 'theme',
 *     labelKey: 'engine_settings.category_theme',
 *     icon: '🎨',
 *     order: 200,
 *     group: 'appearance',
 *   },
 *   component: ThemeSettingsPanel,
 *   defaultData: { currentThemeId: 'default-light', followSystem: false },
 *   onChange: async (newData, oldData) => {
 *     await applyThemeToSDK(newData.currentThemeId);
 *   },
 * };
 * ```
 */
export interface SettingsPanelDefinition<T = unknown> {
  /** 分类元数据（菜单显示用） */
  category: SettingsCategory;
  /** 面板 React 组件 */
  component: ComponentType;
  /** 该分类的默认设置数据 */
  defaultData: T;
  /**
   * 设置变更处理器（可选）
   *
   * 当该分类的数据通过 Store 更新时自动调用。
   * 用于执行副作用，如切换主题调用 SDK.ThemeManager、
   * 切换语言调用 SDK.I18nManager 等。
   *
   * @param newData - 更新后的完整数据
   * @param oldData - 更新前的完整数据
   */
  onChange?(newData: T, oldData: T): void | Promise<void>;
}

// ============================================================
// 各分类的设置数据类型
// ============================================================

/**
 * 语言与文字设置数据
 */
export interface LanguageSettingsData {
  /** 当前语言标识（如 'zh-CN'、'en-US'） */
  locale: string;
  /** 基础字号（px），范围 12-24 */
  fontSize: number;
  /** 内容缩放比例，范围 0.5-2.0 */
  contentScale: number;
}

/**
 * 主题设置数据
 */
export interface ThemeSettingsData {
  /** 当前激活的主题包 ID（每个主题包是独立的，日间和夜间是不同的主题包） */
  currentThemeId: string;
  /** 是否跟随系统主题偏好（自动在亮色/暗色主题包之间切换） */
  followSystem: boolean;
  /** 已安装的主题包 ID 列表 */
  installedThemeIds: string[];
}

/**
 * 引擎模式（布局）设置数据
 */
export interface LayoutSettingsData {
  /** 当前布局模式 */
  currentLayout: string;
}

/**
 * 工具管理设置数据
 *
 * 注意：工具的实际状态（启用/禁用）由 SDK.PluginManager 管理，
 * 此处只存储用户偏好和持久化相关的数据。
 */
export interface ToolsSettingsData {
  /** 用户禁用的工具 ID 列表（持久化用） */
  disabledToolIds: string[];
}

/**
 * 文件管理方式设置数据
 */
export interface FileModeSettingsData {
  /** 文件管理方式：'link' 链接到本地文件 | 'copy' 复制到卡片文件中 */
  fileMode: 'link' | 'copy';
}

/**
 * 关于面板数据
 *
 * 关于面板为纯展示，不需要可修改的设置数据。
 * 此处定义空接口作为保留结构，保持架构一致性。
 */
export interface AboutSettingsData {
  /** 保留字段，关于面板无可修改设置 */
  readonly _placeholder?: never;
}

// ============================================================
// 主题选项（用于 ThemeSettingsPanel 的 UI 展示）
// ============================================================

/**
 * 主题选项
 *
 * ThemeSettingsPanel 中展示可选主题时使用的 UI 数据结构。
 */
export interface ThemeOption {
  /** 主题包 ID */
  id: string;
  /** 主题包名称 */
  name: string;
  /** 主题包类型（亮色/暗色主题包是独立的主题包，不存在模式切换） */
  type: 'light' | 'dark' | 'auto';
  /** 是否为当前选中 */
  isActive: boolean;
  /** 预览色 - 主色 */
  previewPrimary?: string;
  /** 预览色 - 背景色 */
  previewBackground?: string;
  /** 预览色 - 文字色 */
  previewText?: string;
}

// ============================================================
// 设置变更事件
// ============================================================

/**
 * 设置变更事件
 *
 * 通过事件总线发布，供其他模块监听设置变化。
 */
export interface SettingsChangeEvent {
  /** 变更的分类 ID */
  categoryId: SettingsCategoryId;
  /** 变更前的数据 */
  oldData: unknown;
  /** 变更后的数据 */
  newData: unknown;
}
